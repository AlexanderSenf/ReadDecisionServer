 ...
 * @author asenf
 */
public class ReadDecisionServiceHandler extends SimpleChannelInboundHandler<FullHttpRequest> {

    private final BwaMem mem;           // Alternative - simply refer to base object!   
    private final MMapper mask;
    
    public ReadDecisionServiceHandler(BwaMem index, MMapper mask) {
        super();
        this.mem = index;
        this.mask = mask;        
    }
...
//        String unescapedSafeUri = HtmlEscape.unescapeHtml(safeUri);
        String unescapedSafeUri = request.uri();
    ...
    private void sendError(ChannelHandlerContext ctx, HttpResponseStatus status, String get) {
        ...
        try {
            ...
            Logger.getLogger(ReadDecisionServiceHandler.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
